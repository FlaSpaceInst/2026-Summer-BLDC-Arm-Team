...
def runCamera():
    ...
    while True:
        ...
        output, pointCount = drawTracking(frame, priorPoints, curPoints, status, error)

        output, points = calcOpticalFlow(prevFrame, frame)
        cv2.rectangle(output, (x, y), (x + width, y + height), (255, 0, 0), 2) # Highlights selected object
        ...
        if averMove is not None:
            ...
            # Displays average target movement
        ...
        if cv2.waitKey(1) & 0xFF == 27:
            break # Default escape key exit enabled
